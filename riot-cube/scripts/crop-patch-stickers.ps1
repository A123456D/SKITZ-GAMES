# Crop patch sticker sheet (5×3) → public/themes/patch/*.png
Add-Type -AssemblyName System.Drawing

$outDir = Join-Path (Split-Path $PSScriptRoot) "public\themes\patch"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$sheets = @(
  @{
    Path = Join-Path $PSScriptRoot "ref-stickers-patch.png"
    Names = @(
      'pitch', 'grape', 'cherry', 'cobalt', 'blush',
      'tangerine', 'meadow', 'lemon', 'aqua', 'spark',
      'puddle', 'punch', 'washi', 'cocoa', 'sand'
    )
  }
)
...
Write-Host "Done patch stickers."
