# Crop relic sticker sheet (6×4) → public/themes/relic/*.png
Add-Type -AssemblyName System.Drawing

$outDir = Join-Path (Split-Path $PSScriptRoot) "public\themes\relic"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$sheets = @(
  @{
    Path = Join-Path $PSScriptRoot "ref-stickers-relic.png"
    Names = @(
      'bust', 'organ', 'serpent', 'dripface', 'iris', 'koi',
      'moth', 'ukiyo', 'bloom', 'kabuto', 'pillar', 'thunder',
      'regent', 'rose', 'shredder', 'saturn', 'geode', 'cortex',
      'wakeup', 'moon', 'palm', 'atlas', 'omen', 'vial'
    )
  }
)
...
Write-Host "Done relic stickers."
