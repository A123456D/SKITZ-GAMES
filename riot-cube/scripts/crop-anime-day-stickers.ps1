# Crop anime day sticker sheets → public/themes/anime/*.png
# Sheet A: 6 in a row · Sheet B: 2×4 → ANIME_STICKER_POOL filenames.
Add-Type -AssemblyName System.Drawing

$outDir = Join-Path (Split-Path $PSScriptRoot) "public\themes\anime"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$sheets = @(
  @{
    Path = Join-Path $PSScriptRoot "ref-stickers-anime-day-a.png"
    Names = @(
      'bear', 'bolt', 'butterfly', 'candle', 'chain', 'eye'
    )
  },
  @{
    Path = Join-Path $PSScriptRoot "ref-stickers-anime-day-b.png"
    Names = @(
      'eyepatch', 'grimoire', 'heart', 'hood',
      'hourglass', 'katana', 'tears', 'tv'
    )
  }
)
...
function Crop-Sheet([string]$srcPath, [string[]]$names, [string]$outDir) {
  ...
  # Keep the largest stickers only (ignore floating sparkles / bolts).
...
Write-Host "Done anime day stickers."
