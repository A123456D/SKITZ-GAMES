# Crop doodle sticker sheet (5×7) → public/themes/doodle/*.png
Add-Type -AssemblyName System.Drawing

$outDir = Join-Path (Split-Path $PSScriptRoot) "public\themes\doodle"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$sheets = @(
  @{
    Path = Join-Path $PSScriptRoot "ref-stickers-doodle.png"
    Names = @(
      'slime', 'jelly', 'grump', 'drop', 'fire', 'soot', 'goo',
      'starry', 'ice', 'sprout', 'bee', 'bun', 'triple', 'bat',
      'spook', 'stalks', 'cloud', 'meltblob', 'bumpy', 'sheep', 'night',
      'purr', 'uni', 'splash', 'dino', 'bow', 'toad', 'pebble',
      'rainbow', 'reaper', 'king', 'angel', 'urchin', 'swirl', 'devil'
    )
  }
)
...
function Crop-Sheet([string]$srcPath, [string[]]$names, [string]$outDir) {
  ...
  $totalPixels = $w * $h
  $areaThreshold = [int]($totalPixels * 0.0015)
...
Write-Host "Done doodle stickers."
