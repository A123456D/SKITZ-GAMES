# Crop anime sticker sheet v2 → public/themes/anime/*.png
# Uses whole-sheet connected-component detection (irregular rows, ~26 stickers).
Add-Type -AssemblyName System.Drawing

$srcPath = Join-Path $PSScriptRoot "ref-stickers-anime-v2.png"
$outDir = Join-Path (Split-Path $PSScriptRoot) "public\themes\anime"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null

$names = @(
  'skull', 'flame', 'heart', 'mask', 'bolt', 'star',
  'smiley', 'headphones', 'diamond', 'eye', 'spray', 'bomb',
  'cat', 'control', 'tv', 'skate', 'panda', 'crow', 'can',
  'soccer', 'juice', 'sneaker', 'coffee', 'pack', 'reader', 'crown'
)
...
# 1) Flood-fill sheet black from edges → alpha 0
