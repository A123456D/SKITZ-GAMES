/**
 * Install Forge board v2 + process piece singles (gray-bg knockouts → 256² PNG).
 */
...
const OUT_PIECES = join(root, "public", "themes", "forge", "pieces");
const OUT_BOARD = join(root, "public", "themes", "forge", "board.png");
const SOURCE = join(root, "assets", "source", "forge-v2");
const CANVAS = 256;
const PAD = 8;
const BOARD_INSET = 48;
...
async function processOne(name) {
  const src = join(ASSETS, `forge-${name}.png`);
...
async function installBoard() {
  ...
  await sharp(src)
    .extract({
      left: Math.floor(((meta.width || side) - side) / 2) + inset,
      top: Math.floor(((meta.height || side) - side) / 2) + inset,
      width: crop,
      height: crop,
    })
    .resize(1024, 1024)
  ...
  console.log(`board: inset ${inset} → 1024 → ${OUT_BOARD}`);
}
...
console.log("Forge board + pieces installed.");
